...
![막지 상품 6종](../../assets/images/bread-market-products-v1.png)
...
상한(%p)      = min(38, 마진 상한)                # 마진 자료가 없는 상품은 38
할인율(%p)    = clamp(검색쿠폰 + 환율조정, 0, 상한)

판매가 = round(기준가 × (1 − 할인율/100) / 10) × 10
```

- 환율 구간은 오전장이 전영업일 시가→종가, 오후장이 전영업일 종가→당일 시가입니다. v1.0과의 차이는 [docs/산식-버전.md](docs/산식-버전.md)에 있습니다.
- 흐름도는 [F2. 할인율·판매가 계산](../../docs/diagrams/04-flowcharts.md#f2-할인율판매가-계산)을 보세요.
...
자세한 근거는 [docs/PRD-브레드마켓.md](../../docs/PRD-브레드마켓.md) §8, 할인율 결정 과정은 [research/할인율-결정-리포트.md](../../research/할인율-결정-리포트.md)에 있습니다.
...
docs/               앱 운영 가이드·산식 버전·상품 검색어 (기획·정책 문서는 저장소 루트 docs/)
reports/            시스템 테스트·코드 품질·고객 데이터 분석 보고서
```

## 문서

- [docs/PRD-브레드마켓.md](../../docs/PRD-브레드마켓.md) — 제품 기준 문서. 충돌하면 이 문서가 우선합니다.
- [knowledge-base/산식과-쿠폰-공부노트.md](../../knowledge-base/산식과-쿠폰-공부노트.md) — 산식·쿠폰을 근거와 코드 위치까지 이어 놓은 노트
- [docs/DESIGN.md](../../docs/DESIGN.md) — 브랜드·IA·컴포넌트·접근성 기준
...
- 미결 항목은 [docs/DESIGN.md](../../docs/DESIGN.md)의 Open questions와 PRD §24에 있습니다.
