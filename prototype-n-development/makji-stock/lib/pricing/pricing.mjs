...
export function buildSessionFxSignals({ closesByDate, opensByDate, publishDates }) {
  ...
  return Object.fromEntries(
    publishDates.map((publishDate) => {
      ...
      const latestClose = latestRatesBefore(closes, previousCalendarDate, 1)[0];

      /* 두 장이 시간을 이어 덮는다. 겹치지도 비지도 않는다.

           전영업일 시가 ──[오전장]──> 전영업일 종가 ──[오후장]──> 당일 시가

         오전장이 전전영업일 종가에서 출발하던 때는 그 구간이 전날 오후장과
         겹쳐서, 같은 환율 움직임이 이틀에 걸쳐 두 번 가격에 반영됐다.

         오전가 크론은 05시대에 돈다. 전영업일 시가는 그 전날 오전에 이미 나와
         있으므로 늦게 도착할 걱정이 없다. */
      const previousOpen = latestClose ? opensByDate[latestClose[0]] : undefined;
      const morning = !latestClose || !Number.isFinite(previousOpen) || previousOpen <= 0
        ? null
        : {
            reference: "PREVIOUS_OPEN_TO_PREVIOUS_CLOSE",
            previousDate: latestClose[0],
            previousRate: previousOpen,
            currentDate: latestClose[0],
            currentRate: latestClose[1],
            declinePct: ((previousOpen - latestClose[1]) / previousOpen) * 100,
            carriedForward: latestClose[0] !== previousCalendarDate,
          };
